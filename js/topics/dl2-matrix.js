...
a1_1 = max(0, z1_1)</div>`.replace(/,/g, ',').replace(/(\d),(\d)/g, '$1.$2');
  ...
  BrainForge.registerTopic({
    id: 'dl2',
    intro: "Eine Schicht mit vielen Neuronen einzeln zu rechnen ist mühsam. Deshalb packt man alle Werte in <b>Vektoren und Matrizen</b>: eine Formel für die ganze Schicht.<br><br><b>Was wird behandelt?</b> Die Schreibweise (Skalar kursiv, Vektor fett klein, Matrix fett groß), die Schichtformel <b>z</b>⁽ˡ⁾ = <b>W</b>⁽ˡ⁾<b>a</b>⁽ˡ⁻¹⁾ + <b>b</b>⁽ˡ⁾, Dimensionen, der Forward Pass in Python – und die Kernidee: die Zahlen in W und b bestimmen, welche Funktion das Netz berechnet.<br><br><b>Wofür braucht man das?</b> So schreibt und programmiert man Netze in der Praxis – PyTorch, TensorFlow & Co. machen intern genau diese Matrixrechnung. Und die Frage „Woher kommen gute W und b?“ führt direkt zum Thema Lernen.",
    ...
    color: '#7c5cff',
    sheet: [
      { id: 'schreibweise', title: 'Schreibweise', rows: [
        { f: 'Skalar: kursiver Kleinbuchstabe (xᵢ, z<sub>j</sub><sup>(l)</sup>)', d: 'eine einzelne Zahl' },
        { f: 'Vektor: fetter Kleinbuchstabe (<b>x</b>, <b>z</b>⁽ˡ⁾, <b>b</b>⁽ˡ⁾)', d: '' },
        { f: 'Matrix: fetter Großbuchstabe (<b>W</b>⁽ˡ⁾)', d: '' },
        { f: '<b>a</b>⁽⁰⁾ = <b>x</b>', d: 'Eingabevektor = „nullte Schicht“' },
      ]},
      { id: 'schichtformel', title: 'Schichtformel & Dimensionen', rows: [
        { f: '<b>z</b>⁽ˡ⁾ = <b>W</b>⁽ˡ⁾ · <b>a</b>⁽ˡ⁻¹⁾ + <b>b</b>⁽ˡ⁾', d: 'ganze Schicht in einer Formel' },
        { f: '<b>a</b>⁽ˡ⁾ = φ(<b>z</b>⁽ˡ⁾)', d: 'φ wird elementweise auf jeden Eintrag angewendet' },
        { f: '<b>W</b>⁽ˡ⁾ ist n<sub>l</sub> × n<sub>l−1</sub>', d: 'Zeile j = Gewichte von Neuron j · Spalte i = Eingang i' },
        { f: '<b>b</b>, <b>z</b>, <b>a</b>⁽ˡ⁾ haben je n<sub>l</sub> Einträge', d: 'ein Bias / eine Voraktivierung pro Neuron' },
      ]},
      { id: 'rechnen', title: 'Rechnen & Code', rows: [
        { f: 'z<sub>j</sub> = (Zeile j von W) · <b>a</b> + b<sub>j</sub>', d: 'Matrix·Vektor = jede Zeile einzeln wie ein Neuron rechnen' },
        { f: 'Python: z = w1*x1 + w2*x2 + … + b', d: 'gewichtete Summe plus Bias' },
        { f: 'Python ReLU: a = max(0, z)', d: 'Schwelle wäre: 1 if z &gt; 0 else 0 · Identität: a = z' },
      ]},
      { id: 'wb', title: 'W & b bestimmen die Funktion', rows: [
        { f: 'Die Werte in <b>W</b> und <b>b</b> legen fest, welche Funktion das Netz berechnet', d: 'Merksatz aus dem Skript' },
        { f: 'ŷᵢ = m·xᵢ + c für alle i → W = Diagonalmatrix mit m, b-Vektor voller c', d: 'Diagonale: jede Ausgabe hängt nur vom Eingang derselben Position ab' },
        { f: 'Idee des Lernens: Netz findet W und b anhand von Beispielen selbst', d: 'bei komplexen Zusammenhängen nicht mehr von Hand bestimmbar' },
      ]},
    ],
    categories: [
      { id: 'mx-notation', primer: {"what": "Die Schreib-Konvention: Skalare kursiv klein (x<sub>i</sub>), Vektoren <b>fett klein</b> (<b>x</b>), Matrizen <b>fett groß</b> (<b>W</b>). Und der Sonderfall <b>a</b>⁽⁰⁾ = <b>x</b>.", "why": "Damit man in jeder Formel sofort erkennt: Ist das eine einzelne Zahl, eine Liste von Zahlen oder eine ganze Tabelle? Das verhindert Rechenfehler und ist Voraussetzung für alles Weitere.", "ex": "z<sub>1</sub>⁽¹⁾ (kursiv, mit Index) = <b>eine</b> Voraktivierung. <b>z</b>⁽¹⁾ (fett) = <b>alle</b> Voraktivierungen der Schicht 1 auf einmal, als Vektor."}, sheetRef: 'schreibweise', title: 'Schreibweise', desc: 'Skalar, Vektor, Matrix erkennen', tier: 1, generate: genNotation },
      { id: 'mx-konzept', primer: {"what": "Das große Bild: Eine Schicht ist eine Funktion (Eingabevektor rein, Ausgabevektor raus) – und die Zahlen in W und b legen fest, <b>welche</b> Funktion das ist.", "why": "Das ist die Kernidee, auf der später das Lernen aufbaut: Training heißt nichts anderes, als gute Werte für W und b zu finden.", "ex": "Sollen alle Eingaben verdoppelt und um 3 erhöht werden (ŷᵢ = 2xᵢ + 3), schreibt man 2en auf die Diagonale von W und 3en in den Bias-Vektor – fertig ist die Funktion."}, sheetRef: 'wb', title: 'Forward Pass verstehen', desc: 'Schicht als Funktion, W & b', tier: 1, generate: genConcept },
      { id: 'mx-vecact', primer: {"what": "Die Aktivierungsfunktion wird <b>elementweise</b> auf einen Vektor angewendet: jeder Eintrag einzeln durch φ.", "why": "In der Matrixschreibweise steht a⁽ˡ⁾ = φ(z⁽ˡ⁾) – das sieht nach einem Schritt aus, sind aber n einzelne Anwendungen. Wer das weiß, macht bei gemischten Vorzeichen keine Fehler.", "ex": "ReLU auf den Vektor (−2 | 3 | 0):<br><span class=\"formula\">φ((−2|3|0)) = (0 | 3 | 0)</span> – nur der negative Eintrag wird abgeschnitten."}, sheetRef: 'schichtformel', title: 'Elementweise Aktivierung', desc: 'φ auf einen Vektor anwenden', tier: 1, weight: 1.2, generate: genVecAct },
      { id: 'mx-dimension', primer: {"what": "Wie groß sind W, b und z? W⁽ˡ⁾ ist n<sub>l</sub> × n<sub>l−1</sub> (Zeile = Neuron, Spalte = Eingang), b und z haben n<sub>l</sub> Einträge.", "why": "Falsche Dimensionen = Matrix und Vektor passen nicht zusammen. Das ist Fehlerquelle Nr. 1 beim Programmieren (und eine schnelle Punktefrage in der Klausur).", "ex": "3 Eingänge, 2 Neuronen → W ist 2 × 3: zwei Zeilen (eine pro Neuron) mit je drei Gewichten (eins pro Eingang). b und z haben je 2 Einträge."}, sheetRef: 'schichtformel', title: 'Dimensionen', desc: 'Wie groß sind W, b, z?', tier: 2, generate: genDim },
      { id: 'mx-zeile', primer: {"what": "Matrix·Vektor entzaubert: Zeile j von W mal Vektor a, plus b<sub>j</sub> – das ist exakt die alte Neuron-Rechnung, nur kompakt notiert.", "why": "Wer das einmal verstanden hat, hat vor keiner Matrixgleichung mehr Respekt: z = Wa + b ist einfach „jedes Neuron einzeln rechnen“, untereinander geschrieben.", "ex": "Zeile 2 von W ist (1 | −1), a = (2 | 3), b₂ = 1:<br><span class=\"formula\">z₂ = 1·2 + (−1)·3 + 1 = 0</span>"}, sheetRef: 'rechnen', title: 'Matrixzeile rechnen', desc: 'z_j aus W·a + b bestimmen', tier: 2, weight: 1.3, generate: genRow },
      { id: 'mx-code', primer: {"what": "Der Forward Pass als Python-Programm – wie in der Bildungscampus-Aufgabe: z per gewichteter Summe, ReLU als max(0, z).", "why": "Formeln in Code übersetzen (und Code zurück in Formeln lesen) ist eine Kernkompetenz des Fachs – und zeigt, dass hinter „KI“ ganz normale Rechnungen stecken.", "ex": "<span class=\"mono\">z1_1 = w11_1*x1 + w12_1*x2 + b1_1</span> berechnet die Voraktivierung; <span class=\"mono\">a1_1 = max(0, z1_1)</span> ist die ReLU-Aktivierung."}, sheetRef: 'rechnen', title: 'Forward Pass im Code', desc: 'Python-Programm lesen & rechnen', tier: 2, generate: genCode },
      { id: 'mx-schicht', primer: {"what": "Den kompletten Vektor z⁽¹⁾ = W⁽¹⁾a⁽⁰⁾ + b⁽¹⁾ ausrechnen – Zeile für Zeile.", "why": "So rechnet man eine ganze Schicht auf einen Schlag. Das ist die Standard-Rechenaufgabe zur Matrixform.", "ex": "W = ((2, 0), (1, −1)), a = (1 | 2), b = (0 | 1):<br><span class=\"formula\">z₁ = 2·1 + 0·2 + 0 = 2 · z₂ = 1·1 + (−1)·2 + 1 = 0</span> → z = (2 | 0)"}, sheetRef: 'rechnen', title: 'Ganze Schicht rechnen', desc: 'z⁽¹⁾ = W a + b komplett', tier: 3, weight: 1.3, generate: genLayer },
      { id: 'mx-muster', primer: {"what": "Aus (Eingabe → Ausgabe)-Wertepaaren den Zusammenhang ablesen, z.B. „jede Zahl wird verdoppelt und plus 3 gerechnet“.", "why": "Genau das soll ein Netz beim Training <b>selbst</b> herausfinden – hier machst du es einmal von Hand und verstehst dadurch, was „das Netz lernt eine Funktion“ konkret bedeutet.", "ex": "Folie: 2 → 7, 0 → 3, 4 → 11. Differenz pro Schritt: +2 pro +1 → Steigung 2; bei x = 0 kommt 3 raus → ŷ = 2x + 3."}, sheetRef: 'wb', title: 'Zusammenhang erkennen', desc: 'Aus x/ŷ-Paaren die Funktion ablesen', tier: 3, generate: genPattern },
      { id: 'mx-funktion', primer: {"what": "Eine gewünschte Funktion (z.B. ŷᵢ = 1,5·xᵢ − 2) in konkrete Werte für W und b übersetzen.", "why": "Der Beweis der Kernidee in Zahlen: Die Funktion des Netzes <b>ist</b> W und b. Gleichzeitig übt es die Diagonal-Logik: gleiche Position → Diagonale, Konstante → Bias.", "ex": "ŷᵢ = 1,5xᵢ − 2 bei 2 Ein-/Ausgaben: W = ((1,5, 0), (0, 1,5)), b = (−2 | −2). Die Nullen bedeuten: fremde Eingaben mischen nicht mit."}, sheetRef: 'wb', title: 'Funktion → Gewichte', desc: 'y = mx + c in W und b übersetzen', tier: 3, generate: genFn2W },
      { id: 'boss-matrix', primer: {"what": "Die ganze Schicht inklusive Aktivierung: erst z = Wx + b (Zeile für Zeile), dann ReLU elementweise auf jedes z.", "why": "Das ist ein kompletter Schicht-Durchlauf, wie ihn PyTorch millionenfach macht – als Klausuraufgabe in vier Schritten.", "ex": "z = (3 | −2) nach der Matrixrechnung → ReLU macht daraus a = (3 | 0). Erst rechnen, dann aktivieren – nie andersherum!"}, sheetRef: 'rechnen', title: 'Boss: Schicht komplett', desc: 'W·x + b und ReLU, 4 Schritte', tier: 3, boss: true, generate: genBoss },
    ],
  });
})();
